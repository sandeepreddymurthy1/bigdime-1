...
;
;try {
/* module-key = 'com.atlassian.auiplugin:internal-ajs-no-context-path', location = 'js/raphael/raphael.shadow.js' */
...
(function(){window.$=AJS.$;if("undefined"!=typeof AJS){var d=AJS.populateParameters;AJS.populateParameters=function(){d.apply(AJS,arguments);AJS.$("meta[name^=ajs-]").each(function(){var a=this.name,b=this.content,a=a.substring(4).replace(/(-\w)/g,function(a){return a.charAt(1).toUpperCase()});"undefined"==typeof AJS.params[a]&&(AJS.params[a]=AJS.asBooleanOrString(b))})}}AJS.$.fn.disable=function(){return this.each(function(){var a=AJS.$(this),b=a.prop("disabled",!0).addClass("disabled").attr("id");
b&&AJS.$("label[for="+b+"]",a.parent()).addClass("disabled")})};AJS.$.fn.enable=function(){return this.each(function(){var a=AJS.$(this),b=a.attr("disabled",!1).removeClass("disabled").attr("id");b&&AJS.$("label[for="+b+"]",a.parent()).removeClass("disabled")})};AJS.$.debounce=function(a,b){var c;return function(){var d=this,e=arguments;c&&(clearTimeout(c),c=void 0);c=setTimeout(function(){a.apply(d,e);c=void 0},b)}}})();
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
beforeShow:null,onSelect:null,onChangeMonthYear:null,onClose:null,numberOfMonths:1,showCurrentAtPos:0,stepMonths:1,stepBigMonths:12,altField:"",altFormat:"",constrainInput:!0,showButtonPanel:!1,autoSize:!1};d.extend(this._defaults,this.regional[""]);this.dpDiv=d('<div id="'+this._mainDivId+'" class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>')}function y(a,b){d.extend(a,b);for(var c in b)if(null==b[c]||void 0==b[c])a[c]=b[c];return a}d.extend(d.ui,{datepicker:{version:"1.8.11"}});
var s=(new Date).getTime();d.extend(A.prototype,{markerClassName:"hasDatepicker",log:function(){this.debug&&console.log.apply("",arguments)},_widgetDatepicker:function(){return this.dpDiv},setDefaults:function(a){y(this._defaults,a||{});return this},_attachDatepicker:function(a,b){var c=null,e;for(e in this._defaults){var f=a.getAttribute("date:"+e);if(f){c=c||{};try{c[e]=eval(f)}catch(i){c[e]=f}}}e=a.nodeName.toLowerCase();f="div"==e||"span"==e;a.id||(this.uuid+=1,a.id="dp"+this.uuid);var j=this._newInst(d(a),
f);j.settings=d.extend({},b||{},c||{});"input"==e?this._connectDatepicker(a,j):f&&this._inlineDatepicker(a,j)},_newInst:function(a,b){return{id:a[0].id.replace(/([^A-Za-z0-9_-])/g,"\\\\$1"),input:a,selectedDay:0,selectedMonth:0,selectedYear:0,drawMonth:0,drawYear:0,inline:b,dpDiv:!b?this.dpDiv:d('<div class="'+this._inlineClass+' ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>')}},_connectDatepicker:function(a,b){var c=d(a);b.append=d([]);b.trigger=d([]);c.hasClass(this.markerClassName)||
...
a.unbind("focus",this._showDatepicker);b.trigger&&b.trigger.remove();c=this._get(b,"showOn");("focus"==c||"both"==c)&&a.focus(this._showDatepicker);if("button"==c||"both"==c){var c=this._get(b,"buttonText"),f=this._get(b,"buttonImage");b.trigger=d(this._get(b,"buttonImageOnly")?d("<img/>").addClass(this._triggerClass).attr({src:f,alt:c,title:c}):d('<button type="button"></button>').addClass(this._triggerClass).html(""==f?c:d("<img/>").attr({src:f,alt:c,title:c})));a[e?"before":"after"](b.trigger);
b.trigger.click(function(){d.datepicker._datepickerShowing&&d.datepicker._lastInput==a[0]?d.datepicker._hideDatepicker():d.datepicker._showDatepicker(a[0]);return false})}},_autoSize:function(a){if(this._get(a,"autoSize")&&!a.inline){var b=new Date(2009,11,20),c=this._get(a,"dateFormat");if(c.match(/[DM]/)){var d=function(a){for(var b=0,c=0,d=0;d<a.length;d++)a[d].length>b&&(b=a[d].length,c=d);return c};b.setMonth(d(this._get(a,c.match(/MM/)?"monthNames":"monthNamesShort")));b.setDate(d(this._get(a,
...
b,c,e,f){a=this._dialogInst;a||(this.uuid+=1,this._dialogInput=d('<input type="text" id="dp'+this.uuid+'" style="position: absolute; top: -100px; width: 0px; z-index: -10;"/>'),this._dialogInput.keydown(this._doKeyDown),d("body").append(this._dialogInput),a=this._dialogInst=this._newInst(this._dialogInput,!1),a.settings={},d.data(this._dialogInput[0],"datepicker",a));y(a.settings,e||{});b=b&&b.constructor==Date?this._formatDate(a,b):b;this._dialogInput.val(b);this._pos=f?f.length?f:[f.pageX,f.pageY]:
...
a)return!0;return!1},_getInst:function(a){try{return d.data(a,"datepicker")}catch(b){throw"Missing instance data for this datepicker";}},_optionDatepicker:function(a,b,c){var e=this._getInst(a);if(2==arguments.length&&"string"==typeof b)return"defaults"==b?d.extend({},d.datepicker._defaults):e?"all"==b?d.extend({},e.settings):this._get(e,b):null;var f=b||{};"string"==typeof b&&(f={},f[b]=c);if(e){this._curInst==e&&this._hideDatepicker();var i=this._getDateDatepicker(a,!0),j=this._getMinMaxDate(e,
"min"),g=this._getMinMaxDate(e,"max");y(e.settings,f);null!==j&&(void 0!==f.dateFormat&&void 0===f.minDate)&&(e.settings.minDate=this._formatDate(e,j));null!==g&&(void 0!==f.dateFormat&&void 0===f.maxDate)&&(e.settings.maxDate=this._formatDate(e,g));this._attachments(d(a),e);this._autoSize(e);this._setDateDatepicker(a,i);this._updateDatepicker(e)}},_changeDatepicker:function(a,b,c){this._optionDatepicker(a,b,c)},_refreshDatepicker:function(a){(a=this._getInst(a))&&this._updateDatepicker(a)},_setDateDatepicker:function(a,
...
":not(."+d.datepicker._currentClass+")",b.dpDiv),c[0]?d.datepicker._selectDay(a.target,b.selectedMonth,b.selectedYear,c[0]):d.datepicker._hideDatepicker(),!1;case 27:d.datepicker._hideDatepicker();break;case 33:d.datepicker._adjustDate(a.target,a.ctrlKey?-d.datepicker._get(b,"stepBigMonths"):-d.datepicker._get(b,"stepMonths"),"M");break;case 34:d.datepicker._adjustDate(a.target,a.ctrlKey?+d.datepicker._get(b,"stepBigMonths"):+d.datepicker._get(b,"stepMonths"),"M");break;case 35:(a.ctrlKey||a.metaKey)&&
d.datepicker._clearDate(a.target);c=a.ctrlKey||a.metaKey;break;case 36:(a.ctrlKey||a.metaKey)&&d.datepicker._gotoToday(a.target);c=a.ctrlKey||a.metaKey;break;case 37:if(a.ctrlKey||a.metaKey)d.datepicker._adjustDate(a.target,e?1:-1,"D");c=a.ctrlKey||a.metaKey;a.originalEvent.altKey&&d.datepicker._adjustDate(a.target,a.ctrlKey?-d.datepicker._get(b,"stepBigMonths"):-d.datepicker._get(b,"stepMonths"),"M");break;case 38:(a.ctrlKey||a.metaKey)&&d.datepicker._adjustDate(a.target,-7,"D");c=a.ctrlKey||a.metaKey;
break;case 39:if(a.ctrlKey||a.metaKey)d.datepicker._adjustDate(a.target,e?-1:1,"D");c=a.ctrlKey||a.metaKey;a.originalEvent.altKey&&d.datepicker._adjustDate(a.target,a.ctrlKey?+d.datepicker._get(b,"stepBigMonths"):+d.datepicker._get(b,"stepMonths"),"M");break;case 40:(a.ctrlKey||a.metaKey)&&d.datepicker._adjustDate(a.target,7,"D");c=a.ctrlKey||a.metaKey;break;default:c=!1}else 36==a.keyCode&&a.ctrlKey?d.datepicker._showDatepicker(this):c=!1;c&&(a.preventDefault(),a.stopPropagation())},_doKeyPress:function(a){var b=
d.datepicker._getInst(a.target);if(d.datepicker._get(b,"constrainInput")){var b=d.datepicker._possibleChars(d.datepicker._get(b,"dateFormat")),c=String.fromCharCode(void 0==a.charCode?a.keyCode:a.charCode);return a.ctrlKey||a.metaKey||" ">c||!b||-1<b.indexOf(c)}},_doKeyUp:function(a){a=d.datepicker._getInst(a.target);if(a.input.val()!=a.lastVal)try{if(d.datepicker.parseDate(d.datepicker._get(a,"dateFormat"),a.input?a.input.val():null,d.datepicker._getFormatConfig(a)))d.datepicker._setDateFromField(a),
d.datepicker._updateAlternate(a),d.datepicker._updateDatepicker(a)}catch(b){d.datepicker.log(b)}return!0},_showDatepicker:function(a){a=a.target||a;"input"!=a.nodeName.toLowerCase()&&(a=d("input",a.parentNode)[0]);if(!(d.datepicker._isDisabledDatepicker(a)||d.datepicker._lastInput==a)){var b=d.datepicker._getInst(a);d.datepicker._curInst&&d.datepicker._curInst!=b&&d.datepicker._curInst.dpDiv.stop(!0,!0);var c=d.datepicker._get(b,"beforeShow");y(b.settings,c?c.apply(a,[a,b]):{});b.lastVal=null;d.datepicker._lastInput=
a;d.datepicker._setDateFromField(b);d.datepicker._inDialog&&(a.value="");d.datepicker._pos||(d.datepicker._pos=d.datepicker._findPos(a),d.datepicker._pos[1]+=a.offsetHeight);var e=!1;d(a).parents().each(function(){e=e|d(this).css("position")=="fixed";return!e});e&&d.browser.opera&&(d.datepicker._pos[0]-=document.documentElement.scrollLeft,d.datepicker._pos[1]-=document.documentElement.scrollTop);c={left:d.datepicker._pos[0],top:d.datepicker._pos[1]};d.datepicker._pos=null;b.dpDiv.empty();b.dpDiv.css({position:"absolute",
display:"block",top:"-1000px"});d.datepicker._updateDatepicker(b);c=d.datepicker._checkOffset(b,c,e);b.dpDiv.css({position:d.datepicker._inDialog&&d.blockUI?"static":e?"fixed":"absolute",display:"none",left:c.left+"px",top:c.top+"px"});if(!b.inline){var c=d.datepicker._get(b,"showAnim"),f=d.datepicker._get(b,"duration"),i=function(){d.datepicker._datepickerShowing=true;var a=b.dpDiv.find("iframe.ui-datepicker-cover");if(a.length){var c=d.datepicker._getBorders(b.dpDiv);a.css({left:-c[0],top:-c[1],
width:b.dpDiv.outerWidth(),height:b.dpDiv.outerHeight()})}};b.dpDiv.zIndex(d(a).zIndex()+1);if(d.effects&&d.effects[c])b.dpDiv.show(c,d.datepicker._get(b,"showOptions"),f,i);else b.dpDiv[c||"show"](c?f:null,i);(!c||!f)&&i();b.input.is(":visible")&&!b.input.is(":disabled")&&b.input.focus();d.datepicker._curInst=b}}},_updateDatepicker:function(a){var b=this,c=d.datepicker._getBorders(a.dpDiv);a.dpDiv.empty().append(this._generateHTML(a));var e=a.dpDiv.find("iframe.ui-datepicker-cover");e.length&&e.css({left:-c[0],
top:-c[1],width:a.dpDiv.outerWidth(),height:a.dpDiv.outerHeight()});a.dpDiv.find("button, .ui-datepicker-prev, .ui-datepicker-next, .ui-datepicker-calendar td a").bind("mouseout",function(){d(this).removeClass("ui-state-hover");-1!=this.className.indexOf("ui-datepicker-prev")&&d(this).removeClass("ui-datepicker-prev-hover");-1!=this.className.indexOf("ui-datepicker-next")&&d(this).removeClass("ui-datepicker-next-hover")}).bind("mouseover",function(){if(!b._isDisabledDatepicker(a.inline?a.dpDiv.parent()[0]:
a.input[0]))d(this).parents(".ui-datepicker-calendar").find("a").removeClass("ui-state-hover"),d(this).addClass("ui-state-hover"),-1!=this.className.indexOf("ui-datepicker-prev")&&d(this).addClass("ui-datepicker-prev-hover"),-1!=this.className.indexOf("ui-datepicker-next")&&d(this).addClass("ui-datepicker-next-hover")}).end().find("."+this._dayOverClass+" a").trigger("mouseover").end();c=this._getNumberOfMonths(a);e=c[1];1<e?a.dpDiv.addClass("ui-datepicker-multi-"+e).css("width",17*e+"em"):a.dpDiv.removeClass("ui-datepicker-multi-2 ui-datepicker-multi-3 ui-datepicker-multi-4").width("");
a.dpDiv[(1!=c[0]||1!=c[1]?"add":"remove")+"Class"]("ui-datepicker-multi");a.dpDiv[(this._get(a,"isRTL")?"add":"remove")+"Class"]("ui-datepicker-rtl");a==d.datepicker._curInst&&(d.datepicker._datepickerShowing&&a.input&&a.input.is(":visible")&&!a.input.is(":disabled")&&a.input[0]!=document.activeElement)&&a.input.focus();if(a.yearshtml){var f=a.yearshtml;setTimeout(function(){f===a.yearshtml&&a.dpDiv.find("select.ui-datepicker-year:first").replaceWith(a.yearshtml);f=a.yearshtml=null},0)}},_getBorders:function(a){var b=
function(a){return{thin:1,medium:2,thick:3}[a]||a};return[parseFloat(b(a.css("border-left-width"))),parseFloat(b(a.css("border-top-width")))]},_checkOffset:function(a,b,c){var e=a.dpDiv.outerWidth(),f=a.dpDiv.outerHeight(),i=a.input?a.input.outerWidth():0,j=a.input?a.input.outerHeight():0,g=document.documentElement.clientWidth+d(document).scrollLeft(),h=document.documentElement.clientHeight+d(document).scrollTop();b.left-=this._get(a,"isRTL")?e-i:0;b.left-=c&&b.left==a.input.offset().left?d(document).scrollLeft():
0;b.top-=c&&b.top==a.input.offset().top+j?d(document).scrollTop():0;b.left-=Math.min(b.left,b.left+e>g&&g>e?Math.abs(b.left+e-g):0);b.top-=Math.min(b.top,b.top+f>h&&h>f?Math.abs(f+j):0);return b},_findPos:function(a){for(var b=this._getInst(a),b=this._get(b,"isRTL");a&&("hidden"==a.type||1!=a.nodeType||d.expr.filters.hidden(a));)a=a[b?"previousSibling":"nextSibling"];a=d(a).offset();return[a.left,a.top]},_hideDatepicker:function(a){var b=this._curInst;if(b&&!(a&&b!=d.data(a,"datepicker"))&&this._datepickerShowing){var a=
this._get(b,"showAnim"),c=this._get(b,"duration"),e=function(){d.datepicker._tidyDialog(b);this._curInst=null};if(d.effects&&d.effects[a])b.dpDiv.hide(a,d.datepicker._get(b,"showOptions"),c,e);else b.dpDiv["slideDown"==a?"slideUp":"fadeIn"==a?"fadeOut":"hide"](a?c:null,e);a||e();(a=this._get(b,"onClose"))&&a.apply(b.input?b.input[0]:null,[b.input?b.input.val():"",b]);this._datepickerShowing=!1;this._lastInput=null;this._inDialog&&(this._dialogInput.css({position:"absolute",left:"0",top:"-100px"}),
...
10),f=(c?c.dayNamesShort:null)||this._defaults.dayNamesShort,i=(c?c.dayNames:null)||this._defaults.dayNames,j=(c?c.monthNamesShort:null)||this._defaults.monthNamesShort,g=(c?c.monthNames:null)||this._defaults.monthNames,h=c=-1,p=-1,r=-1,k=!1,m=function(b){(b=s+1<a.length&&a.charAt(s+1)==b)&&s++;return b},l=function(a){var c=m(a),a=RegExp("^\\d{1,"+("@"==a?14:"!"==a?20:"y"==a&&c?4:"o"==a?3:2)+"}"),a=b.substring(w).match(a);if(!a)throw"Missing number at position "+w;w+=a[0].length;return parseInt(a[0],
10)},n=function(a,c,d){a=m(a)?d:c;for(c=0;c<a.length;c++)if(b.substr(w,a[c].length).toLowerCase()==a[c].toLowerCase())return w+=a[c].length,c+1;throw"Unknown name at position "+w;},t=function(){if(b.charAt(w)!=a.charAt(s))throw"Unexpected literal at position "+w;w++},w=0,s=0;s<a.length;s++)if(k)"'"==a.charAt(s)&&!m("'")?k=!1:t();else switch(a.charAt(s)){case "d":p=l("d");break;case "D":n("D",f,i);break;case "o":r=l("o");break;case "m":h=l("m");break;case "M":h=n("M",j,g);break;case "y":c=l("y");break;
case "@":var v=new Date(l("@")),c=v.getFullYear(),h=v.getMonth()+1,p=v.getDate();break;case "!":v=new Date((l("!")-this._ticksTo1970)/1E4);c=v.getFullYear();h=v.getMonth()+1;p=v.getDate();break;case "'":m("'")?t():k=!0;break;default:t()}-1==c?c=(new Date).getFullYear():100>c&&(c+=(new Date).getFullYear()-(new Date).getFullYear()%100+(c<=d?0:-100));if(-1<r){h=1;p=r;do{d=this._getDaysInMonth(c,h-1);if(p<=d)break;h++;p-=d}while(1)}v=this._daylightSavingAdjust(new Date(c,h-1,p));if(v.getFullYear()!=c||
v.getMonth()+1!=h||v.getDate()!=p)throw"Invalid date";return v},ATOM:"yy-mm-dd",COOKIE:"D, dd M yy",ISO_8601:"yy-mm-dd",RFC_822:"D, d M y",RFC_850:"DD, dd-M-y",RFC_1036:"D, d M y",RFC_1123:"D, d M yy",RFC_2822:"D, d M yy",RSS:"D, d M y",TICKS:"!",TIMESTAMP:"@",W3C:"yy-mm-dd",_ticksTo1970:864E9*(718685+Math.floor(492.5)-Math.floor(19.7)+Math.floor(4.925)),formatDate:function(a,b,c){if(!b)return"";var d=(c?c.dayNamesShort:null)||this._defaults.dayNamesShort,f=(c?c.dayNames:null)||this._defaults.dayNames,
i=(c?c.monthNamesShort:null)||this._defaults.monthNamesShort,c=(c?c.monthNames:null)||this._defaults.monthNames,j=function(b){(b=r+1<a.length&&a.charAt(r+1)==b)&&r++;return b},g=function(a,b,c){b=""+b;if(j(a))for(;b.length<c;)b="0"+b;return b},h="",p=!1;if(b)for(var r=0;r<a.length;r++)if(p)"'"==a.charAt(r)&&!j("'")?p=!1:h+=a.charAt(r);else switch(a.charAt(r)){case "d":h+=g("d",b.getDate(),2);break;case "D":var k;k=b.getDay();var m=d,l=f;k=j("D")?l[k]:m[k];h+=k;break;case "o":h+=g("o",(b.getTime()-
(new Date(b.getFullYear(),0,0)).getTime())/864E5,3);break;case "m":h+=g("m",b.getMonth()+1,2);break;case "M":k=b.getMonth();m=i;l=c;k=j("M")?l[k]:m[k];h+=k;break;case "y":h+=j("y")?b.getFullYear():(10>b.getYear()%100?"0":"")+b.getYear()%100;break;case "@":h+=b.getTime();break;case "!":h+=1E4*b.getTime()+this._ticksTo1970;break;case "'":j("'")?h+="'":p=!0;break;default:h+=a.charAt(r)}return h},_possibleChars:function(a){for(var b="",c=!1,d=function(b){(b=f+1<a.length&&a.charAt(f+1)==b)&&f++;return b},
f=0;f<a.length;f++)if(c)"'"==a.charAt(f)&&!d("'")?c=!1:b+=a.charAt(f);else switch(a.charAt(f)){case "d":case "m":case "y":case "@":b+="0123456789";break;case "D":case "M":return null;case "'":d("'")?b+="'":c=!0;break;default:b+=a.charAt(f)}return b},_get:function(a,b){return void 0!==a.settings[b]?a.settings[b]:this._defaults[b]},_setDateFromField:function(a,b){if(a.input.val()!=a.lastVal){var c=this._get(a,"dateFormat"),d=a.lastVal=a.input?a.input.val():null,f,i;f=i=this._getDefaultDate(a);var j=
this._getFormatConfig(a);try{f=this.parseDate(c,d,j)||i}catch(g){this.log(g),d=b?"":d}a.selectedDay=f.getDate();a.drawMonth=a.selectedMonth=f.getMonth();a.drawYear=a.selectedYear=f.getFullYear();a.currentDay=d?f.getDate():0;a.currentMonth=d?f.getMonth():0;a.currentYear=d?f.getFullYear():0;this._adjustInstDate(a)}},_getDefaultDate:function(a){return this._restrictMinMax(a,this._determineDate(a,this._get(a,"defaultDate"),new Date))},_determineDate:function(a,b,c){if(null==b||""===b)b=c;else{var e;if("string"==
typeof b)a:{try{e=d.datepicker.parseDate(d.datepicker._get(a,"dateFormat"),b,d.datepicker._getFormatConfig(a));break a}catch(f){}var i=(b.toLowerCase().match(/^c/)?d.datepicker._getDate(a):null)||new Date,a=i.getFullYear();e=i.getMonth();for(var i=i.getDate(),j=/([+-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g,g=j.exec(b);g;){switch(g[2]||"d"){case "d":case "D":i+=parseInt(g[1],10);break;case "w":case "W":i+=7*parseInt(g[1],10);break;case "m":case "M":e+=parseInt(g[1],10);i=Math.min(i,d.datepicker._getDaysInMonth(a,
e));break;case "y":case "Y":a+=parseInt(g[1],10),i=Math.min(i,d.datepicker._getDaysInMonth(a,e))}g=j.exec(b)}e=new Date(a,e,i)}else"number"==typeof b?isNaN(b)?b=c:(a=new Date,a.setDate(a.getDate()+b),b=a):b=new Date(b.getTime()),e=b;b=e}if(b=b&&"Invalid Date"==b.toString()?c:b)b.setHours(0),b.setMinutes(0),b.setSeconds(0),b.setMilliseconds(0);return this._daylightSavingAdjust(b)},_daylightSavingAdjust:function(a){if(!a)return null;a.setHours(12<a.getHours()?a.getHours()+2:0);return a},_setDate:function(a,
b,c){var d=!b,f=a.selectedMonth,i=a.selectedYear,b=this._restrictMinMax(a,this._determineDate(a,b,new Date));a.selectedDay=a.currentDay=b.getDate();a.drawMonth=a.selectedMonth=a.currentMonth=b.getMonth();a.drawYear=a.selectedYear=a.currentYear=b.getFullYear();(f!=a.selectedMonth||i!=a.selectedYear)&&!c&&this._notifyChange(a);this._adjustInstDate(a);a.input&&a.input.val(d?"":this._formatDate(a))},_getDate:function(a){return!a.currentYear||a.input&&""==a.input.val()?null:this._daylightSavingAdjust(new Date(a.currentYear,
a.currentMonth,a.currentDay))},_generateHTML:function(a){var b=new Date,b=this._daylightSavingAdjust(new Date(b.getFullYear(),b.getMonth(),b.getDate())),c=this._get(a,"isRTL"),e=this._get(a,"showButtonPanel"),f=this._get(a,"hideIfNoPrevNext"),i=this._get(a,"navigationAsDateFormat"),j=this._getNumberOfMonths(a),g=this._get(a,"showCurrentAtPos"),h=this._get(a,"stepMonths"),p=1!=j[0]||1!=j[1],r=this._daylightSavingAdjust(!a.currentDay?new Date(9999,9,9):new Date(a.currentYear,a.currentMonth,a.currentDay)),
k=this._getMinMaxDate(a,"min"),m=this._getMinMaxDate(a,"max"),g=a.drawMonth-g,l=a.drawYear;0>g&&(g+=12,l--);if(m)for(var n=this._daylightSavingAdjust(new Date(m.getFullYear(),m.getMonth()-j[0]*j[1]+1,m.getDate())),n=k&&n<k?k:n;this._daylightSavingAdjust(new Date(l,g,1))>n;)g--,0>g&&(g=11,l--);a.drawMonth=g;a.drawYear=l;var n=this._get(a,"prevText"),n=!i?n:this.formatDate(n,this._daylightSavingAdjust(new Date(l,g-h,1)),this._getFormatConfig(a)),n=this._canAdjustMonth(a,-1,l,g)?'<a class="ui-datepicker-prev ui-corner-all" onclick="DP_jQuery_'+
s+".datepicker._adjustDate('#"+a.id+"', -"+h+", 'M');\" title=\""+n+'"><span class="ui-icon ui-icon-circle-triangle-'+(c?"e":"w")+'">'+n+"</span></a>":f?"":'<a class="ui-datepicker-prev ui-corner-all ui-state-disabled" title="'+n+'"><span class="ui-icon ui-icon-circle-triangle-'+(c?"e":"w")+'">'+n+"</span></a>",t=this._get(a,"nextText"),t=!i?t:this.formatDate(t,this._daylightSavingAdjust(new Date(l,g+h,1)),this._getFormatConfig(a)),f=this._canAdjustMonth(a,1,l,g)?'<a class="ui-datepicker-next ui-corner-all" onclick="DP_jQuery_'+
s+".datepicker._adjustDate('#"+a.id+"', +"+h+", 'M');\" title=\""+t+'"><span class="ui-icon ui-icon-circle-triangle-'+(c?"w":"e")+'">'+t+"</span></a>":f?"":'<a class="ui-datepicker-next ui-corner-all ui-state-disabled" title="'+t+'"><span class="ui-icon ui-icon-circle-triangle-'+(c?"w":"e")+'">'+t+"</span></a>",h=this._get(a,"currentText"),t=this._get(a,"gotoCurrent")&&a.currentDay?r:b,h=!i?h:this.formatDate(h,t,this._getFormatConfig(a)),i=!a.inline?'<button type="button" class="ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all" onclick="DP_jQuery_'+
s+'.datepicker._hideDatepicker();">'+this._get(a,"closeText")+"</button>":"",e=e?'<div class="ui-datepicker-buttonpane ui-widget-content">'+(c?i:"")+(this._isInRange(a,t)?'<button type="button" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" onclick="DP_jQuery_'+s+".datepicker._gotoToday('#"+a.id+"');\">"+h+"</button>":"")+(c?"":i)+"</div>":"",i=parseInt(this._get(a,"firstDay"),10),i=isNaN(i)?0:i,h=this._get(a,"showWeek"),t=this._get(a,"dayNames");this._get(a,"dayNamesShort");
var w=this._get(a,"dayNamesMin"),y=this._get(a,"monthNames"),v=this._get(a,"monthNamesShort"),A=this._get(a,"beforeShowDay"),F=this._get(a,"showOtherMonths"),M=this._get(a,"selectOtherMonths");this._get(a,"calculateWeek");for(var J=this._getDefaultDate(a),B="",C=0;C<j[0];C++){for(var G="",D=0;D<j[1];D++){var K=this._daylightSavingAdjust(new Date(l,g,a.selectedDay)),u=" ui-corner-all",q="";if(p){q+='<div class="ui-datepicker-group';if(1<j[1])switch(D){case 0:q+=" ui-datepicker-group-first";u=" ui-corner-"+
(c?"right":"left");break;case j[1]-1:q+=" ui-datepicker-group-last";u=" ui-corner-"+(c?"left":"right");break;default:q+=" ui-datepicker-group-middle",u=""}q+='">'}for(var q=q+('<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix'+u+'">'+(/all|left/.test(u)&&0==C?c?f:n:"")+(/all|right/.test(u)&&0==C?c?n:f:"")+this._generateMonthYearHeader(a,g,l,k,m,0<C||0<D,y,v)+'</div><table class="ui-datepicker-calendar"><thead><tr>'),x=h?'<th class="ui-datepicker-week-col">'+this._get(a,"weekHeader")+
"</th>":"",u=0;7>u;u++)var o=(u+i)%7,x=x+("<th"+(5<=(u+i+6)%7?' class="ui-datepicker-week-end"':"")+'><span title="'+t[o]+'">'+w[o]+"</span></th>");q+=x+"</tr></thead><tbody>";x=this._getDaysInMonth(l,g);l==a.selectedYear&&g==a.selectedMonth&&(a.selectedDay=Math.min(a.selectedDay,x));for(var u=(this._getFirstDayOfMonth(l,g)-i+7)%7,x=p?6:Math.ceil((u+x)/7),o=this._daylightSavingAdjust(new Date(l,g,1-u)),L=0;L<x;L++){for(var q=q+"<tr>",H=!h?"":'<td class="ui-datepicker-week-col">'+this._get(a,"calculateWeek")(o)+
"</td>",u=0;7>u;u++){var E=A?A.apply(a.input?a.input[0]:null,[o]):[!0,""],z=o.getMonth()!=g,I=z&&!M||!E[0]||k&&o<k||m&&o>m,H=H+('<td class="'+(5<=(u+i+6)%7?" ui-datepicker-week-end":"")+(z?" ui-datepicker-other-month":"")+(o.getTime()==K.getTime()&&g==a.selectedMonth&&a._keyEvent||J.getTime()==o.getTime()&&J.getTime()==K.getTime()?" "+this._dayOverClass:"")+(I?" "+this._unselectableClass+" ui-state-disabled":"")+(z&&!F?"":" "+E[1]+(o.getTime()==r.getTime()?" "+this._currentClass:"")+(o.getTime()==
b.getTime()?" ui-datepicker-today":""))+'"'+((!z||F)&&E[2]?' title="'+E[2]+'"':"")+(I?"":' onclick="DP_jQuery_'+s+".datepicker._selectDay('#"+a.id+"',"+o.getMonth()+","+o.getFullYear()+', this);return false;"')+">"+(z&&!F?"&#xa0;":I?'<span class="ui-state-default">'+o.getDate()+"</span>":'<a class="ui-state-default'+(o.getTime()==b.getTime()?" ui-state-highlight":"")+(o.getTime()==r.getTime()?" ui-state-active":"")+(z?" ui-priority-secondary":"")+'" href="#">'+o.getDate()+"</a>")+"</td>");o.setDate(o.getDate()+
1);o=this._daylightSavingAdjust(o)}q+=H+"</tr>"}g++;11<g&&(g=0,l++);q+="</tbody></table>"+(p?"</div>"+(0<j[0]&&D==j[1]-1?'<div class="ui-datepicker-row-break"></div>':""):"");G+=q}B+=G}B+=e+(d.browser.msie&&7>parseInt(d.browser.version,10)&&!a.inline?'<iframe src="javascript:false;" class="ui-datepicker-cover" frameborder="0"></iframe>':"");a._keyEvent=!1;return B},_generateMonthYearHeader:function(a,b,c,e,f,i,j,g){var h=this._get(a,"changeMonth"),p=this._get(a,"changeYear"),r=this._get(a,"showMonthAfterYear"),
k='<div class="ui-datepicker-title">',m="";if(i||!h)m+='<span class="ui-datepicker-month">'+j[b]+"</span>";else{for(var j=e&&e.getFullYear()==c,l=f&&f.getFullYear()==c,m=m+('<select class="ui-datepicker-month" onchange="DP_jQuery_'+s+".datepicker._selectMonthYear('#"+a.id+"', this, 'M');\" onclick=\"DP_jQuery_"+s+".datepicker._clickMonthYear('#"+a.id+"');\">"),n=0;12>n;n++)if((!j||n>=e.getMonth())&&(!l||n<=f.getMonth()))m+='<option value="'+n+'"'+(n==b?' selected="selected"':"")+">"+g[n]+"</option>";
m+="</select>"}r||(k+=m+(i||!h||!p?"&#xa0;":""));a.yearshtml="";if(i||!p)k+='<span class="ui-datepicker-year">'+c+"</span>";else{var g=this._get(a,"yearRange").split(":"),t=(new Date).getFullYear(),j=function(a){a=a.match(/c[+-].*/)?c+parseInt(a.substring(1),10):a.match(/[+-].*/)?t+parseInt(a,10):parseInt(a,10);return isNaN(a)?t:a},b=j(g[0]),g=Math.max(b,j(g[1]||"")),b=e?Math.max(b,e.getFullYear()):b,g=f?Math.min(g,f.getFullYear()):g;for(a.yearshtml+='<select class="ui-datepicker-year" onchange="DP_jQuery_'+
s+".datepicker._selectMonthYear('#"+a.id+"', this, 'Y');\" onclick=\"DP_jQuery_"+s+".datepicker._clickMonthYear('#"+a.id+"');\">";b<=g;b++)a.yearshtml+='<option value="'+b+'"'+(b==c?' selected="selected"':"")+">"+b+"</option>";a.yearshtml+="</select>";d.browser.mozilla?k+='<select class="ui-datepicker-year"><option value="'+c+'" selected="selected">'+c+"</option></select>":(k+=a.yearshtml,a.yearshtml=null)}k+=this._get(a,"yearSuffix");r&&(k+=(i||!h||!p?"&#xa0;":"")+m);return k+"</div>"},_adjustInstDate:function(a,
...
d.fn.datepicker=function(a){if(!this.length)return this;d.datepicker.initialized||(d(document).mousedown(d.datepicker._checkExternalClick).find("body").append(d.datepicker.dpDiv),d.datepicker.initialized=!0);var b=Array.prototype.slice.call(arguments,1);return"string"==typeof a&&("isDisabled"==a||"getDate"==a||"widget"==a)||"option"==a&&2==arguments.length&&"string"==typeof arguments[1]?d.datepicker["_"+a+"Datepicker"].apply(d.datepicker,[this[0]].concat(b)):this.each(function(){typeof a=="string"?
d.datepicker["_"+a+"Datepicker"].apply(d.datepicker,[this].concat(b)):d.datepicker._attachDatepicker(this,a)})};d.datepicker=new A;d.datepicker.initialized=!1;d.datepicker.uuid=(new Date).getTime();d.datepicker.version="1.8.11";window["DP_jQuery_"+s]=d})(jQuery);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
jQuery(function(a){var b=["Su","Mo","Tu","We","Th","Fr","Sa"],c=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],
d=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],e=["January","February","March","April","May","June","July","August","September","October","November",
"December"],f=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];a.datepicker.regional[""]={closeText:"Done",prevText:"Prev",
nextText:"Next",currentText:"Today",monthNames:e,monthNamesShort:f,dayNames:d,dayNamesShort:c,dayNamesMin:b,weekHeader:"wk",dateFormat:a.datepicker.ISO_8601,firstDay:1,isRTL:!1,showMonthAfterYear:!1,yearSuffix:""};a.datepicker.setDefaults(a.datepicker.regional[""])});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
Raphael.spinner=function(f,b,d){for(var g=d||"#fff",a=13*b/60,d=35*b/60,i=b+a,j=b+a,h=Raphael(f,2*b+2*a,2*b+2*a),k=[],e=[],f=2*Math.PI/12,g={stroke:g,"stroke-width":a,"stroke-linecap":"round"},a=0;12>a;a++){var c=f*a-Math.PI/2,l=Math.cos(c),c=Math.sin(c);e[a]=a/12;k[a]=h.path([["M",i+d*l,j+d*c],["L",i+b*l,j+b*c]]).attr(g)}var m;(function n(){e.unshift(e.pop());for(var a=0;12>a;a++)k[a].attr("opacity",e[a]);h.safari();m=setTimeout(n,80)})();return function(){clearTimeout(m);h.remove()}};
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
(function(h){var m=function(c,f){f=f||{};setTimeout(function(){var d=AJS.Rte.Content.offset(c.anchor),i=c.panel.width(),b=i+d.left-h(window).width()+10,a=h(c.anchor).outerHeight(),b=d.left-(0<b?b:0)-0,a=c.shouldFlip?d.top-7-c.panel.outerHeight()-4:d.top+7+a;if(c.options.anchorIframe)var g=h(c.options.anchorIframe),g=g.offset().top+g.height()-c.panel.outerHeight()-10,a=Math.min(a,g);c.panel.find(".aui-tip-parent").css({left:Math.min(Math.abs(d.left-b)+10,i-12)});b=Math.max(0,b);d={top:a,left:b};i=
c.panel.add(c.shadow);b=function(){c.shadow&&c.shadow.css("left",c.panel.position().left-1)};f.animate?i.animate(d,f.animateDuration,b):(i.css(d),b())},f.delay||0)};AJS.Confluence.PropertyPanel={shouldCreate:!0,current:null,createFromButtonModel:function(c,f,d,i){for(var b=AJS("div").attr({"class":"panel-buttons"}),a=0,g=d.length;a<g;a++)if(d[a]){var e=d[a],j=e.html||'<span class="icon"></span>',k=[];e.text&&(j+='<span class="panel-button-text">'+e.text+"</span>");e.className&&k.push(e.className);
e.disabled&&k.push("disabled");e.selected&&k.push("selected");!d[a+1]&&k.push("last");!d[a-1]&&k.push("first");e.html?j=h(e.html):(j=AJS("a").attr({href:d[a].href||"#"}).addClass("aui-button").html(j),e.disabled?(j.attr("title",e.disabledText),j.disable(),j.click(function(a){return AJS.stopEvent(a)})):d[a].click&&function(a,b,c){b.click(function(d){a.click(b,c);return AJS.stopEvent(d)})}(d[a],j,f));e.tooltip&&j.attr("data-tooltip",e.tooltip);j.addClass(k.join(" "));b.append(j)}return this.create(c,
f,b,i)},create:function(c,f,d,i){i=i||{};AJS.Rte.BookmarkManager.storeBookmark();var b=h("#property-panel"),a,g=void 0==i.enableFlip||i.enableFlip;b.length&&this.destroy();b=AJS("div").addClass("aui-property-panel-parent").addClass(c+"-panel").attr("id","property-panel").appendTo("body");a=AJS("div").addClass("aui-property-panel").append(d);b.append(a).css({top:0,left:-1E4});if(a=g){var g=h(i.anchorIframe||h(f).parent()),e=h(f);a=b.outerHeight()+10;g=AJS.Position.spaceAboveBelow(g[0],e);a=g.below>=
a?!1:g.above>=a}var j=this;d.find(".last:last").css({"margin-right":0});var d=Raphael.shadow(0,0,b.outerWidth(!0),b.outerHeight(!0),{radius:5,target:b[0],offset:2}),g=b,e=-13,k=h('<div class="aui-tip-parent"></div>');h.browser.msie&&9>h.browser.version&&(e=-15);var l=a?14:7;g.prepend(k).css({zIndex:3001});var l=Raphael(k[0],15,l),n=l.path("M0,6.0001l6.00001-6.00001,6.0001,6.0001").attr({fill:"#f5f5f5",stroke:"#e2e2e2"});l.canvas.style.zIndex=3E3;tip=l;a&&(n.rotate(180),h.support.opacity&&n.clone().translate(2,
3).attr({fill:"#A0A0A0",stroke:"#A0A0A0",opacity:".5",blur:"1"}).toBack(),e=e+g.outerHeight()+11);k.css({top:e,left:10});this.current={anchor:f,panel:b,hasAnchorChanged:function(a){return a&&j.hasAnchorChanged(a)},snapToElement:function(a){m(this,a)},shouldFlip:a,shadow:d,tip:l,options:i,updating:!0,type:c};m(this.current);a=this.current;AJS.$(document).bind("keydown.property-panel.escape",function(a){a.keyCode===27&&AJS.Confluence.PropertyPanel.destroy()});AJS.$(document).bind("click.property-panel",
...
this.current=null):AJS.log("PropertyPanel.destroy: called with no current PropertyPanel, returning")},hasAnchorChanged:function(c){var f=this.current;return f&&h(f.anchor)[0]==h(c)[0]?f.options.originalHeight&&f.options.originalHeight!=h(c).height():!0}}})(AJS.$);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
AJS.toInit(function(b){if(Confluence.Templates.Labels){var c=null,h=function(){b("#"+c.id).find(".label-list").removeClass("editable");c.hide();return!1};b("#rte-button-labels").live("click",function(){AJS.Labels.openDialog()});b(".show-labels-editor").click(function(a){a.preventDefault();a=b(a.target).closest(".labels-section-content");AJS.Labels.openDialog(a)});AJS.Labels.bindOpenDialog=function(a){a.click(function(a){a.preventDefault();a=b(a.target).closest(".labels-section-content");a.length?
AJS.Labels.openDialog(a):AJS.Labels.openDialog()})};AJS.Labels.openDialog=function(a){var f,d,e;c||(c=AJS.ConfluenceDialog({width:600,height:250,id:"edit-labels-dialog",onCancel:h}),c.addHeader("Labels"),c.addPanel("Label Editor",Confluence.Templates.Labels.dialog()),c.addCancel("Close",h),c.addHelpText("Shortcut tip: In page view, pressing \u003cb\u003e{shortcut}\u003c/b\u003e also opens this dialog box",{shortcut:"l"}),c.popup.element.find(".dialog-title").prepend(Confluence.Templates.Labels.helpLink()),b("#add-labels-form").submit(function(a){var c=
b("#labels-string");a.preventDefault();a=b("#dialog-label-list");AJS.Labels.addLabel(c.val(),a.attr("entityid"),a.attr("entitytype"));c.focus();setTimeout(function(){b("#labels-autocomplete-list").find(".aui-dropdown").addClass("hidden")})}),AJS.Labels.bindAutocomplete());a?(d=a.attr("entityid"),f=a.attr("entitytype")):(f=AJS.Meta.get("content-type"),d=AJS.Meta.get("page-id"));a=AJS.Labels.routes.list(f,d);e=b("#dialog-label-list");e.attr("entityid",d);e.attr("entitytype",f);var g=function(){c.show();
b("#labels-string").val("").focus();AJS.trigger("labels.dialog.open")};d=function(a){e.html(Confluence.Templates.Labels.dialogLabelList({labels:a.labels,spaceKey:AJS.Meta.get("space-key"),editable:true}));g()};AJS.Labels.isNewPage()?e.find("ul").children().size()?g():(a=b("#createPageLabelsString").val()||"",a=AJS.Labels.parseLabelStringToArray(a),d({labels:a})):AJS.$.ajax({url:a,cache:!1,success:d,error:function(){AJS.Labels.setLabelError("Unable to fetch current labels from the server.");g()}});
b("#dialog-label-list").attr("data-ready","true")}}});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
Confluence.Templates.Labels.helpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "https://docs.atlassian.com/confluence/docs-57/Add%2C+Remove+and+Search+for+Labels"}, output);
  return opt_sb ? '' : output.toString();
};
...
AJS.Labels=function(b){function p(a,g){if(!a||!g)var d=b("#dialog-label-list"),a=d.attr("entityid"),g=d.attr("entitytype");return!a||!g?b(i.labelView):b(".labels-section-content").filter(function(){return this.getAttribute("entityid")==a&&this.getAttribute("entitytype")==g}).find(i.labelView)}AJS.bind("init.rte-control",function(){var a=0;AJS.Meta.get("num-labels")?a=AJS.Meta.get("num-labels"):b("#createPageLabelsString").val()&&(a=b.trim(b("#createPageLabelsString").val()).split(/\s+/).length);0!=
a&&(AJS.Meta.set("num-labels",a),b("#rte-button-labels").trigger("updateLabel"))});var r=function(a){a=AJS.$.trim(a);if(!a)return[];var g=[],d=(new Date).getTime();b(a.split(/\s+/)).each(function(a,b){var e=b.split(":"),f,j=e[0];2===e.length&&(f=e[0],j=e[1]);g.push({name:j,prefix:f,id:d});d++});return g},i={labelView:".label-list",labelItem:".aui-label",noLabelsMessage:".no-labels-message",idAttribute:"data-label-id",labelsFormFieldId:AJS.Meta.get("labels-form-field-id")||"createPageLabelsString"},
s=Confluence.getContextPath(),m={base:s+"/rest/ui/1.0/",getRestEndPoint:function(a){return this.base+("attachment"==a?"attachment/":"template"==a?"template/":"space"==a?"space/":"content/")},index:s+"/labels/autocompletelabel.action?maxResults=3",validate:function(){return this.getRestEndPoint()+"labels/validate"},add:function(a,b){return this.getRestEndPoint(a)+b+"/labels"},remove:function(a,b,d){return this.getRestEndPoint(a)+b+"/label/"+d},list:function(a,b){return this.getRestEndPoint(a)+b+"/labels"}},
k=function(){return!!document.getElementById("createpageform")||!!document.getElementById("createpagetemplate")},q=function(){return b(".space-administration").length},t=function(a){n();a&&a.promise&&(a.always([u,v]),a.done(function(a,d){AJS.Meta.set("num-labels",p().first().find(i.labelItem).size());b("#rte-button-labels").trigger("updateLabel");var c=d.not(".editable");0===d.find(".aui-label").length?c.find(".labels-edit-container").before(Confluence.Templates.Labels.nolabels()):c.find(".no-labels-message").remove()}));
return a},v=function(){b("#labels-string, #add-labels-editor-button").removeAttr("disabled aria-disabled")},u=function(){b("#labels-string").val("")},n=function(a){a=a||null;b("#label-operation-error-message").empty().toggle(!!a);AJS.messages.warning("#label-operation-error-message",{body:a})};b(".label-list.editable .aui-icon-close").live("click",function(a){a.preventDefault();a=b("#dialog-label-list");AJS.Labels.removeLabel(b(this).closest("li"),a.attr("entityid"),a.attr("entitytype"))});AJS.toInit(function(){q()&&
(AJS.Labels.bindAutocomplete(),b(".label-list").addClass("editable"))});return{addLabel:function(a,g,d){var c=a;if(c){b("#labels-string, #add-labels-editor-button").attr({disabled:"disabled","aria-disabled":!0});var h=b("#"+i.labelsFormFieldId);if(k()){var a=(h.val()+" "+c).split(/\s+/),e=[];AJS.$.each(a,function(a,b){-1===AJS.$.inArray(b,e)&&e.push(b)});c=e.join(" ")}var a=r(c),a={url:k()?m.validate():m.add(d,g),type:"POST",dataType:"json",contentType:"application/json",data:JSON.stringify(a)},f=
b.Deferred(),a=AJS.$.ajax(a);a.done(function(a){var e=b("#dialog-label-list").find(".label-list"),l=p(g,d).not(".editable"),a=a.labels;if(k()){h.val(c);var w=(new Date).getTime();AJS.$.each(a,function(a,b){b.id=w++});e.empty()}e.html(Confluence.Templates.Labels.labels({labels:a,spaceKey:AJS.Meta.get("space-key"),editable:!0}));l.find("li.aui-label").remove();l.find("li.labels-edit-container").before(Confluence.Templates.Labels.labels({labels:a,spaceKey:AJS.Meta.get("space-key")}));f.resolve(a,l.add(e))});
a.fail(function(a,b){var c;if("timeout"===b)c="Connecting to the server timed out, you\'ll need to try again";else try{c=JSON.parse(a.responseText).message}catch(d){c=b,AJS.log("Unexpected error when adding labels: "+d.message)}AJS.log("Failed to add labels: "+c);AJS.log("Failed to add labels xhr: "+a.responseText);n(c);f.reject(b)});a=f.promise()}else a=!1;return t(a)},removeLabel:function(a,g,d){var c=a,h=g,e=d;if(c){c=c.jquery?c:b(c);q()&&(h=AJS.Meta.get("space-key"),e="space");var f=c.attr(i.idAttribute),j=b.trim(c.find("a").text()),
a={type:"DELETE",dataType:"json",contentType:"application/json",data:{}},o=b.Deferred();k()?(a=b.Deferred(),a.resolve()):(a.url=m.remove(e,h,f),a=AJS.$.ajax(a));a.done(function(){var a=p(h,e);q()&&(a=b("#space-categories-list"));var d=a.find(i.labelItem).filter("["+i.idAttribute+"='"+f+"']");d.fadeOut("normal",function(){d.remove();o.resolve(c,a)})});a.fail(function(a,b){AJS.log(b);n(b=="timeout"?"Connecting to the server timed out, you\'ll need to try again":b);o.reject(b)});k()&&o.done(function(){var a=b("#"+i.labelsFormFieldId),
c=a.val().split(/\s+/),c=b.grep(c,function(a){return!a||a==j},true);a.val(c.join(" "))});a=o.promise()}else a=!1;return t(a)},bindAutocomplete:function(){var a=b("#labels-string"),g=a.parents("#add-labels-form").length;a.length&&(b(window).bind("quicksearch.ajax-success",function(a,b){if("/labels/autocompletelabel.action?maxResults=3"==b.url)return AJS.Labels.queryTokens=b.json&&b.json.queryTokens||[],!1}),b(window).bind("quicksearch.ajax-error",function(a,b){if("/labels/autocompletelabel.action?maxResults=3"==
b.url)return AJS.Labels.queryTokens=[],!1}),a.quicksearch("/labels/autocompletelabel.action?maxResults=3",function(){if(!b("#labels-autocomplete-list .label-suggestion").length||""===a.val())this.hide();else if(!g)for(var d=b("#labels-autocomplete-list a.label-suggestion"),c=0,h=d.length;c<h;c++)d.get(c).href="#"},{makeParams:function(a){return{query:a,contentId:AJS.params.pageId||""}},dropdownPlacement:function(a){b("#labels-autocomplete-list").append(a)},ajsDropDownOptions:{selectionHandler:function(d,
c){if(c.find("a.label-suggestion").length){var h=b("span",c),h=b.data(h[0],"properties"),e=a.val(),f=[];if(g)f=e.split(/\s+/),f[f.length-1]=h.name,a.val(f.join(" "));else{for(var j=AJS.Labels.queryTokens,f=-1,i,l="",k=0,m=j.length;k<m;k++)l=j[k],i=e.lastIndexOf(l),i>f&&(f=i);-1!=f?(j=e.substr(0,f),(e=e.substr(f+l.length).match(/^\s+/))&&(j+=e[0]),a.val(j+h.name)):a.val(h.name)}}this.hide();d.preventDefault()}}}))},isNewPage:k,routes:m,setLabelError:n,parseLabelStringToArray:r}}(AJS.$);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
Confluence.Templates.ManageWatchers.helpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "https://docs.atlassian.com/confluence/docs-57/Managing+Watchers"}, output);
  return opt_sb ? '' : output.toString();
};
...
(function(d){d.ui=d.ui||{};d.fn.extend({spinner:function(g){if(!this.is(".ui-spinner"))return new d.ui.spinner(this,g||{})}});d.ui.spinner=function(g,e){this.anchor=g;var a=AJS.params.staticResourceUrlPrefix||contextPath;this.images=e.images||[a+"/images/ddtree/black spinner/1.png",a+"/images/ddtree/black spinner/2.png",a+"/images/ddtree/black spinner/3.png",a+"/images/ddtree/black spinner/4.png",a+"/images/ddtree/black spinner/5.png",a+"/images/ddtree/black spinner/6.png",a+"/images/ddtree/black spinner/7.png",
a+"/images/ddtree/black spinner/8.png",a+"/images/ddtree/black spinner/9.png",a+"/images/ddtree/black spinner/10.png",a+"/images/ddtree/black spinner/11.png",a+"/images/ddtree/black spinner/12.png"];this.width=e.width||"16px";this.height=e.height||e.width||"16px";this.hide=function(){this.anchor.hide();this.stop()};this.show=function(){this.start();this.anchor.show()};this.fadeIn=function(){this.anchor.fadeIn.apply(this.anchor,arguments)};this.fadeOut=function(){this.anchor.fadeOut.apply(this.anchor,
arguments)};this.moveTo=function(a,b){this.anchor.css("top",b);this.anchor.css("left",a)};this.putInBox=function(a){var b=a.y||a.y1,c="undefined"==typeof a.height?a.y2-a.y1:a.height;this.moveTo((a.x||a.x1)+Math.round((("undefined"==typeof a.width?a.x2-a.x1:a.width)-this.offsetWidth)/2),b+Math.round((c-this.offsetHeight)/2))};this.start=function(){this.timer||(this.timer=setInterval(h,100));return this.timer};this.stop=function(){clearInterval(this.timer);this.timer=null};this.divs=[];for(var a=0,
i=this.images.length;a<i;a++){var c=document.createElement("div");if(!AJS.applyPngFilter(c,this.images[a])){var f=document.createElement("img");f.src=this.images[a];f.style.width=this.width;f.style.height=this.height;c.appendChild(f)}c.style.width=this.width;c.style.height=this.height;this.anchor.append(c);this.offsetWidth||(this.offsetWidth=c.offsetWidth,this.offsetHeight=c.offsetHeight);this.divs.push(d(c).hide())}this.frame=0;this.direction=1;var b=this,h=function(){b.divs[b.frame].hide();b.frame+=
b.direction;b.frame>=b.divs.length&&(b.frame=0);0>b.frame&&(b.frame=b.divs.length-1);b.divs[b.frame].show()};this.anchor.css("position","absolute")}})(jQuery);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
(function(c){c.ui=c.ui||{};c.fn.extend({tree:function(t){if(!this.is(".ui-tree"))return new c.ui.tree(this,t)}});var u=function(c){c.preventDefault()};c.ui.tree=function(t,k){function v(a,b){for(var a=(a+"").toLowerCase(),b=(b+"").toLowerCase(),d=/(\d+|\D+)/g,e=a.match(d),d=b.match(d),c=Math.max(e.length,d.length),f=0;f<c;f++){if(f==e.length)return-1;if(f==d.length)return 1;var s=parseInt(e[f],10),i=parseInt(d[f],10);if(s==e[f]&&i==d[f]&&s!=i)return(s-i)/Math.abs(s-i);if((s!=e[f]||i!=d[f])&&e[f]!=
d[f])return e[f]<d[f]?-1:1}return 0}function w(a){this[0]=a[0];this.$=a;this.text=a.find("span").text();this.href=a.find("a").attr("href");this.linkClass=a.find("a").attr("class");this.nodeClass=a.attr("class");this.open=function(a){if(b.visibleNodes.length)return b.visibleNodes[this[0].num].open&&b.visibleNodes[this[0].num].open(a);AJS.log("tried to open empty node")};this.insertChild=function(a){a.$&&(a=a[0]);b.visibleNodes[this[0].num].append(a)};this.reorder=function(){b.visibleNodes[this[0].num].order(v)};
this.close=function(){b.visibleNodes[this[0].num].close()};this.getAttribute=function(a){return this[0][a]};this.setAttribute=function(a,b){this[0][a]=b};this.highlight=function(){this.$.addClass("highlighted")};this.unhighlight=function(){this.$.removeClass("highlighted")};this.makeDraggable=function(){this.setAttribute("undraggable",!1);this.$.removeClass("undraggable")};this.makeUndraggable=function(){this.setAttribute("undraggable",!0);this.$.addClass("undraggable")};this.makeClickable=function(a){this.setAttribute("unclickable",
!1);this.$.removeClass("unclickable");var d=this[0].getElementsByTagName("a"),a=a?c(d[0]):c(d);a.unbind("click",u);a.click(b.events.click)};this.makeUnclickable=function(a){this.setAttribute("unclickable",!0);this.$.addClass("unclickable");var d=this[0].getElementsByTagName("a"),a=a?c(d[0]):c(d);a.click(u);a.unbind("click",b.events.click)};this.setText=function(a){this.text=a;this[0].text=a;this.$.find("span").text(a)};this.getParent=function(){if(this.$.parent(":not(.ui-tree)").length){var a=this.$.parent().parent();
if(a.length)return new w(c(a[0]))}return null};this.append=function(a){var b=this.$.find("ul");if(!b.length){if(this[0].toBeLoaded){var d=this;this.open(function(){d.append(a)});return!1}this.$.append("<ul></ul>");b=this.$.find("ul")}var g=q(a);b.append(g);p.call(g);"undefined"==typeof this[0].closed&&(this.$.addClass("closed"),this[0].closed=!0,b.hide());j()};this.below=function(a){a=q(a);this.$.after(a);p.call(a);j()};this.above=function(a){a=q(a);this.$.before(a);p.call(a);j()};this.remove=function(){this.$.remove();
j()};this.reload=function(){this[0].getElementsByTagName("ul").length&&(this[0].removeChild(this[0].getElementsByTagName("ul")[0]),this.$.removeClass("opened").addClass("closed"),this[0].closed=!0,b.visibleNodes[this[0].num].open())};this.order=function(a){var b=c("ul",this.$),d=this[0];d.ordered=!0;if(b.length){var g=[];d.oldorder=[];c("li",this.$).each(function(){g.push(this);d.oldorder.push(this)});g.sort(function(b,d){return a(c(b).find("span").html(),c(d).find("span").html())});d.order=g;for(var i=
0,k=g.length;i<k;i++)b.append(g[i])}j()};this.orderUndo=function(){this[0].ordered=!1;var a=c("ul",this.$);if(this[0].oldorder&&a.length)for(var b=0,d=this[0].oldorder.length;b<d;b++)a.append(this[0].oldorder[b]);this[0].oldorder=null;j()};this.setOrdered=function(a){this[0].ordered=a;c("a.abc:first",this).css("display",a?"none":"block");c("a.rollback:first",this).css("display","none")};if(i.options.parameters&&i.options.parameters.length)for(var g=0,d=i.options.parameters.length;g<d;g++)a[0][i.options.parameters[g]]&&
(this[i.options.parameters[g]]=a[0][i.options.parameters[g]])}function x(a){return!("li"==a.tagName.toLowerCase()&&1>c("li:not(.tree-helper)",a).length)}function m(a){this.$li=c(a);this.height=this.$li.height()}function r(a){a=b.points[a];return"undefined"!=typeof a?{visibleNode:b.visibleNodes[a.num],where:a.where,top:a.top}:{visibleNode:new m(o),where:"append",top:b.dim.top}}function z(){var a=0,g=0;b.points=[];for(var d=0,c=b.visibleNodes.length;d<c;d++){var h=b.visibleNodes[d].$li.offset(),f=Math.round(h.top);
b.visibleNodes[d].top=f;b.visibleNodes[d].left=Math.round(h.left);if(a)for(var h=(f-a)/4,i=a;i<f;i++){var j=i-a<h?"above":i-a<3*h?"append":"below";b.points[i]={num:g,where:j,top:a}}if(d==c-1){h=b.visibleNodes[d].height/4;for(i=f;i<f+b.visibleNodes[d].height;i++)j=i-f<h?"above":i-f<3*h?"append":"below",b.points[i]={num:d,where:j,top:f}}a=f;g=d}}function j(){b.visibleNodes=[];for(var a=c("li:visible",o),g=0,d=a.length;g<d;g++)c(a[g]).hasClass("tree-helper")||(a[g].num=b.visibleNodes.length,b.visibleNodes.push(new m(a[g])));
z()}function p(){var a=c(this);i.options.undraggable?a.mousedown(u):(a.draggable(B()),a[0].undraggable=a.hasClass("undraggable"));var g=c(this.getElementsByTagName("a")[0]);i.options.unclickable?(a.addClass("unclickable"),g.click(u)):g.click(b.events.click);i.options.oninsert&&i.options.oninsert.call(new w(a),g)}function q(a){var b=document.createElement("li");b.className=a.nodeClass;if(i.options.parameters&&i.options.parameters.length)for(var d=0,e=i.options.parameters.length;d<e;d++)a[i.options.parameters[d]]&&
(b[i.options.parameters[d]]=a[i.options.parameters[d]]);i.options.nodeId&&(b.id="node-"+a[i.options.nodeId]);var d=document.createElement("a"),e=document.createElement("span"),h=document.createElement("i");h.className="decorator";d.href=a.href;e.appendChild(document.createTextNode(a.text));d.appendChild(e);d.appendChild(h);d.className=a.linkClass;a=document.createElement("div");c(a).addClass("click-zone");c(a).click(C);c(b).mouseover(D).mouseout(E);b.appendChild(a);b.appendChild(d);d=document.createElement("div");
d.className="button-panel";b.appendChild(d);e=document.createElement("a");e.className="abc";e.title="Sort Alphabetically";d.appendChild(e);h=document.createElement("a");h.className="rollback";h.title="Undo Sorting";d.appendChild(h);c(e).click(F);c(h).click(G);if(i.options.isAdministrator){var f=document.createElement("a");f.className="preview-node";f.title="Preview";d.appendChild(f);c(f).click(H);f=document.createElement("a");f.className="remove-node";f.title="Delete";d.appendChild(f);c(f).click(I)}c(e).css("display",
"none");c(h).css("display","none");d=c(b);d.hasClass("opened")?(d.removeClass("opened").addClass("closed"),b.closed=!0):d.hasClass("closed")?b.toBeLoaded=!0:c(a).css("display","none");return b}var l=t,i=this,A=!1,J=arguments;if(!/^[ou]l$/i.test(l[0].tagName)){A=!0;if(!k.url)return!1;l.html("<ul></ul>");l=c("ul",l)}var o=l[0];l.addClass("ui-tree");var b={list:l,visibleNodes:[],dim:l.offset(),points:[],win:c(window),timer:null,prev:0,events:{grab:function(){},click:function(){},drag:function(){},drop:function(){},
append:function(){},insertabove:function(){},insertbelow:function(){},load:function(){},nodeover:function(){},nodeout:function(){},onready:function(){},order:function(){},orderUndo:function(){},remove:function(){},preview:function(){}}};this.options=k;this.expandPath=function(a,b){b=b||function(){};if(a.length){var d=1,c,h,f=function(){if(d<a.length){for(var h in a[d])if(c=i.findNodeBy(h,a[d][h]))break;d++;c.open(f)}else b()};for(h in a[0]){c=this.findNodeBy(h,a[0][h]);break}c&&c.open(f)}else b()};
this.reload=function(a){A&&l.remove();for(var b in a)this.options[b]=a[b];return new J.callee(t,this.options)};this.append=function(a){a=q(a);l.append(a);p.call(a);j()};this.unhighlight=function(){l.find("li.highlighted").each(function(){c(this).removeClass("highlighted")})};this.findNodeBy=function(a,b){for(var d=[],e=o.getElementsByTagName("li"),h=0,f=e.length;h<f;h++)e[h][a]==b&&d.push(new w(c(e[h])));return 0==d.length?null:1==d.length?d[0]:d};if(k.url){var n=document.createElement("div");n.className=
"tree-spinner";k.spinnerId&&(n.id=k.spinnerId);c("body").append(n);b.spinner=c(n).spinner();b.spinner.hide()}for(var y in b.events)"function"==typeof k[y]&&(b.events[y]=k[y]);m.prototype.append=function(a){if(this.$li[0]==a)return!1;if(this.$li[0].toBeLoaded){var g=this;this.load(function(){g.append(a)});return!1}if("li"==this.$li[0].tagName.toLowerCase()){var d=c("ul:first",this.$li),e=a.parentNode.parentNode;c(".rollback:first",e).css("display","none");d.length?(d.append(a),this.$li[0].ordered&&
this.order(v)):(d=document.createElement("ul"),d.appendChild(a),this.$li[0].appendChild(d),this.$li.addClass("opened"),c(".click-zone:first",this.$li).css("display","inline"),c(".rollback:first",this.$li).css("display","none"));x(e)||b.visibleNodes[e.num].notaFolderAnymore();setTimeout(j,0);b.events.append.call({source:a,target:this.$li[0]})}};m.prototype.below=function(a){var g=a.parentNode.parentNode;this.$li.after(a);c(".rollback:first",g).css("display","none");x(g)?!c(a.parentNode).hasClass("ui-tree")&&
!a.parentNode.parentNode.undraggable&&(a.parentNode.parentNode.ordered=!1,c(".abc:first",a.parentNode.parentNode).css("display","block"),c(".rollback:first",a.parentNode.parentNode).css("display","none")):b.visibleNodes[g.num].notaFolderAnymore();setTimeout(j,0);b.events.insertbelow.call({source:a,target:this.$li[0]})};m.prototype.above=function(a){var g=a.parentNode.parentNode;this.$li.before(a);c(".rollback:first",g).css("display","none");x(g)?!c(a.parentNode).hasClass("ui-tree")&&!a.parentNode.parentNode.undraggable&&
(a.parentNode.parentNode.ordered=!1,c(".abc:first",a.parentNode.parentNode).css("display","block"),c(".rollback:first",a.parentNode.parentNode).css("display","none")):b.visibleNodes[g.num].notaFolderAnymore();setTimeout(j,0);b.events.insertabove.call({source:a,target:this.$li[0]})};m.prototype.order=function(a){var b=this.$li[0];b.ordered=!0;var d=c("ul:first",this.$li);if(d.length){var e=[];b.oldorder=[];c("li",this.$li).each(function(){this.parentNode.parentNode==b&&(e.push(this),b.oldorder.push(this))});
e.sort(function(b,d){var g=c("span",b).text().replace(/^\s+|\s+$/g,""),e=c("span",d).text().replace(/^\s+|\s+$/g,"");return a(g,e)});b.order=e;for(var h=0,f=e.length;h<f;h++)d.append(e[h])}j()};m.prototype.orderUndo=function(){var a=this.$li[0];a.ordered=!1;var b=c("ul:first",this.$li);if(a.oldorder&&b.length&&b[0].parentNode==a)for(var d=0,e=a.oldorder.length;d<e;d++)b.append(a.oldorder[d]);a.oldorder=null;a.oldor=null;j()};m.prototype.open=function(a){a=a||function(){};if(this.$li.hasClass("closed")){var b=
c("ul:has(li)",this.$li);return b.length?(b.show(),this.closed=!1,this.$li.removeClass("closed").addClass("opened"),j(),a(!0),!0):this.load(a)}a(!1);return!1};m.prototype.close=function(a){var a=a||function(){},c=this.$li.contents().filter("ul:has(li)");c.length&&(c.hide(),this.closed=!0,this.$li.removeClass("opened").addClass("closed"),b.visibleNodes.splice(this.$li[0].num+1,c[0].getElementsByTagName("li").length),j(),a())};m.prototype.load=function(a){var g=i.options.url;if(!g)return!1;a=a||function(){};
this.$li[0].toBeLoaded=!1;this.$li[0].closed=!0;var d={};if(k.parameters&&k.parameters.length)for(var e=0,h=k.parameters.length;e<h;e++)d[k.parameters[e]]=this.$li[0][k.parameters[e]]||"";var f=this,h=this.$li[0].getElementsByTagName("span")[0],e=h.offsetWidth,h=Math.round(c(h).offset().left);f.loading=!0;b.spinner.putInBox({x:h+e,y:this.top,width:25,height:b.H});b.spinner.show();c.ajax({url:g,type:"GET",dataType:"json",data:d,success:function(d){var g=c("ul",f.$li);g.length||(g=document.createElement("ul"),
f.$li[0].appendChild(g),g=c(g));f.ordered="number"!=typeof d[0].position;for(var e=0,h=d.length;e<h;e++){var i=q(d[e]);g[0].appendChild(i);p.call(i)}g.hide();f.open(a);b.events.load();b.spinner.hide();f.$li[0].ordered=f.ordered;c(".abc:first",f.$li[0]).css("display",f.ordered||i.undraggable?"none":"block");c(".rollback:first",f.$li[0]).css("display","none")}});return!0};m.prototype.notaFolderAnymore=function(){this.$li.removeClass("closed").removeClass("opened");c(".click-zone:first",this.$li).hide();
c(".abc:first",this.$li).css("display","none");c(".rollback:first",this.$li).css("display","none");var a=this.$li[0].getElementsByTagName("ul");this.closed=!1;a.length&&this.$li[0].removeChild(a[0])};this.updateVisibleNodes=j;var B=function(){var a={distance:3,helper:"clone",opacity:0.7,cursorAt:{top:b.H/2,left:30},stop:function(c){clearInterval(b.timer);clearTimeout(b.opentimer);b.opentimer=null;var d=r(b.prev);d.visibleNode.$li.removeClass("over").removeClass("above").removeClass("append").removeClass("below");
d.visibleNode.$li.next().removeClass("over").removeClass("above").removeClass("append").removeClass("below");b.win.unbind("keypress",b.escape);delete b.escape;if(a.revert)return a.revert=!1;for(var d=r(c.pageY||c.originalEvent.pageY),c=d.visibleNode.$li[0],e=!0;c!=o;){if(c==this){e=!1;break}c=c.parentNode}if(e=e&&!("above"==d.where&&d.visibleNode.$li.prev()[0]==this)&&!("append"==d.where&&d.visibleNode.$li[0]==this.parentNode.parentNode))d.visibleNode[d.where](this),b.events.drop.call({position:d.where,
source:this,target:d.visibleNode.$li[0]})},start:function(g,d){var e=this;d.helper.append("<strong></strong>").addClass("tree-helper").find(".button-panel").remove();b.events.grab.call(e);this.undraggable&&(d.helper.addClass("no"),a.revert=!0);b.escape=function(g){if(g.keyCode==27){g=r(b.prev);g.visibleNode.$li.removeClass("over").removeClass("above").removeClass("append").removeClass("below");g.visibleNode.$li.next().removeClass("over").removeClass("above").removeClass("append").removeClass("below");
var f=d.helper.clone();d.helper.before(f);f.animate({left:Math.round(c(e).offset().left)+"px",top:Math.round(c(e).offset().top)+"px",opacity:0},"slow","swing",function(){f.remove()});d.helper.css("display","none");a.revert=true}};b.win.keypress(b.escape)},drag:function(c,d){c.pageY=c.pageY||c.originalEvent.pageY;c.pageX=c.pageX||c.originalEvent.pageX;var e=r(b.prev);e.visibleNode.$li.removeClass("above").removeClass("append").removeClass("below");e.visibleNode.$li.next().removeClass("above").removeClass("append").removeClass("below");
if(!a.revert||b.out){b.prev=c.pageY;var h=r(b.prev);if(h.visibleNode.$li[0]==o)a.revert=!0,b.out=!0;else{b.out&&(b.out=!1,a.revert=!1);h.visibleNode!=e.visibleNode&&(b.events.nodeout.call(e.visibleNode.$li),b.opentimer&&(clearTimeout(b.opentimer),b.opentimer=!1));b.events.nodeover.call({element:h.visibleNode.$li,position:h.where});var e=h.where,f=h.visibleNode.$li.next();"below"==e&&f.length&&!f.hasClass("tree-helper")?f.addClass("above"):r(b.prev).visibleNode.$li.addClass(e);if("append"==h.where&&
(h.visibleNode.closed||h.visibleNode.$li[0].toBeLoaded)&&!b.opentimer)e=b,f=setTimeout(function(){h.visibleNode.$li.removeClass("append");h.visibleNode.open(function(){b.opentimer=false})},500),e.opentimer=f;var i=arguments.callee;30>b.win.height()-c.pageY+b.win.scrollTop()?(clearInterval(b.timer),b.timer=setInterval(function(){window.scrollBy(0,4);d.helper.css("top",parseInt(d.helper.css("top"))+4+"px");i({pageY:c.pageY+4},d)},b.win.height()-c.pageY+b.win.scrollTop())):0<b.win.scrollTop()&&30>c.pageY-
b.win.scrollTop()?(clearInterval(b.timer),b.timer=setInterval(function(){window.scrollBy(0,-4);i({pageY:c.pageY-4},d);d.helper.css("top",parseInt(d.helper.css("top"))-4+"px")},c.pageY-b.win.scrollTop())):b.timer&&clearInterval(b.timer);b.events.drag.call({element:this,left:c.pageX,top:c.pageY})}}}};return a};c.ui.tree.callNumber=0;var C=function(){if(!b.visibleNodes[this.parentNode.num].loading)return c(this.parentNode).hasClass("closed")?b.visibleNodes[this.parentNode.num].open():b.visibleNodes[this.parentNode.num].close(),
!1},D=function(a){c(a.target).hasClass("tree-helper")||c(".button-panel:first",this).addClass("hover");return!1},E=function(a){c(a.target).hasClass("tree-helper")||c(".button-panel:first",this).removeClass("hover");return!1},F=function(){var a=b.visibleNodes[this.parentNode.parentNode.num];a.order(v);b.events.order.call({source:a.$li[0]});c(this).hide();c("a.rollback",this.parentNode).show();return!1},G=function(){var a=b.visibleNodes[this.parentNode.parentNode.num];a.orderUndo();b.events.orderUndo.call({source:a.$li[0],
orderedChildren:c("ul:first",a.$li[0]).children()});c(this).hide();c("a.abc",this.parentNode).show();return!1},H=function(a){a.preventDefault();b.events.preview.call({source:preview,node:b.visibleNodes[this.parentNode.parentNode.num].$li[0]})},I=function(a){a.preventDefault();b.events.remove.call({source:b.visibleNodes[this.parentNode.parentNode.num].$li[0]})},n=l.contents().filter("li");if(0<n.length)b.H=n.height(),n.each(p),j(),b.events.onready.call(this);else{n=i.options.initUrl||i.options.url;
if(!n)return!1;b.spinner.putInBox({x:b.dim.left+16,y:b.dim.top,width:16,height:16});b.spinner.show();var K=++c.ui.tree.callNumber;c.getJSON(n,function(a){for(var g=0,d=a.length;g<d;g++){var e=q(a[g]);o.appendChild(e);0==g&&(b.H=c(e).height());p.call(e)}j();b.spinner.hide();K==c.ui.tree.callNumber&&(b.events.onready.call(this),c.ui.tree.callNumber=0)})}b.offset=o.offsetTop;setInterval(function(){o.offsetTop!=b.offset&&(z(),b.offset=o.offsetTop)},10);return this}})(jQuery);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
var GADGET_DIRECTORY=null;GadgetDirectory=function(){};
GadgetDirectory.prototype={dlg:null,gadget_list:null,showDialog:function(){this.dlg.addHeader(GADGET_DIRECTORY.translations.header);this.dlg.addCancel(GADGET_DIRECTORY.translations.closeButton,function(a){a.hide();return!1});AJS.$(".throbber").hide();this.dlg.getCurrentPanel().html(this.buildPanel())},buildPanel:function(){return AJS.$("<div/>").addClass("directory-panel").append(this.buildDirectoryHelp()).append(this.buildDirectoryList())},buildDirectoryHelp:function(){var a=AJS.$("<div/>").addClass("directory-help");
AJS.$("<h3/>").addClass("directory-help-title").html(GADGET_DIRECTORY.translations.helpTitle1).appendTo(a);AJS.$("<p/>").addClass("directory-help-text").html(GADGET_DIRECTORY.translations.helpBody1).appendTo(a);AJS.$("<h3/>").addClass("directory-help-title").html(GADGET_DIRECTORY.translations.helpTitle2).appendTo(a);AJS.$("<p/>").addClass("directory-help-text").html(GADGET_DIRECTORY.translations.helpBody2).appendTo(a);AJS.$("<p/>").html(GADGET_DIRECTORY.translations.moreInfo).appendTo(a);return a},
buildDirectoryList:function(){if(!GADGET_DIRECTORY.gadget_list)return AJS.$("<div/>").addClass("no-gadgets").html("No gadgets found.");var a=AJS.$("<ol/>").addClass("macro-list");AJS.$.each(GADGET_DIRECTORY.gadget_list,function(f,b){var c=AJS.$("<li/>").attr("id","macro-"+b.title.split(" ").join("")).addClass("macro-list-item"),g=AJS.$("<h3/>").addClass("macro-title").html(b.title),h=AJS.$("<p/>").append(AJS.$("<span/>").append(AJS.$("<a/>").addClass("macro-uri").attr("href",b.url).attr("target",
"_blank").attr("title",b.url).html(GADGET_DIRECTORY.translations.gadgetUrl))),d=AJS.$("<p/>").addClass("macro-author");b.author?d.html("By "+b.author):d.html(GADGET_DIRECTORY.translations.noAuthor);var e=AJS.$("<p/>").addClass("macro-desc");b.description?e.html(b.description):e.html(GADGET_DIRECTORY.translations.noDescription);if(b.thumbnail){var i=AJS.$("<img/>").addClass("macro-icon").attr({height:60,width:120,alt:"",src:b.thumbnail});c.append(i)}c.append(g);c.append(h);c.append(d);c.append(e);
a.append(c)});return a},buildThrobber:function(){var a=AJS.$("<div/>").addClass("throbber");Raphael.spinner(a[0],60,"#666");return a},loadDirectory:function(){AJS.$(document).keyup(function(a){if(27==a.keyCode)return GADGET_DIRECTORY.dlg.hide(),AJS.$(document).unbind("keyup",arguments.callee),AJS.stopEvent(a)});this.dlg?this.dlg.show():(this.dlg=new AJS.Dialog(865,530,"gadget-directory-dialog"),this.dlg.addPanel("","panel1"),this.dlg.getCurrentPanel().html(this.buildThrobber()),this.dlg.getCurrentPanel().setPadding(0),
this.dlg.getCurrentPanel().body.css("overflow","hidden"),this.dlg.show(),AJS.$.ajax({url:contextPath+"/rest/gadget/1.0/published/gadgetsdirectory",type:"GET",dataType:"json",success:function(a){a.directoryList&&(GADGET_DIRECTORY.gadget_list=a.directoryList);GADGET_DIRECTORY.translations=a.translations;GADGET_DIRECTORY.showDialog()},error:function(){GADGET_DIRECTORY.dlg.getCurrentPanel().html(AJS.$("<div/>").addClass("loading-error").html("An error has occurred while trying to load the gadget directory."))},
timeout:12E3}))}};AJS.toInit(function(a){GADGET_DIRECTORY=new GadgetDirectory;AJS.$("#gadget-directory-link").click(function(f){a(this).removeClass("interactive");GADGET_DIRECTORY.loadDirectory();f.preventDefault()})});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
Confluence.Templates.Blueprints.helpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "https://docs.atlassian.com/confluence/docs-57/Creating+Content"}, output);
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Blueprints.spaceHelpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "https://docs.atlassian.com/confluence/docs-57/Create+a+Space"}, output);
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.KeyboardShortcutsDialog.keyboardShortcutHelpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.customHelpLink({href: soy.$$escapeHtml("https://docs.atlassian.com/confluence/docs-57/Keyboard+Shortcuts"), text: soy.$$escapeHtml("View All Shortcuts")}, output);
  return opt_sb ? '' : output.toString();
};
